package com.callv2.dns.manager.domain.validation.handler;
...
public class Notification implements ValidationHandler {
    ...
    public static Notification create(final Throwable t) {
        return new Notification(new ArrayList<>()).append(ValidationError.with(t.getMessage()));
    }
    ...
    public <T> T valdiate(final Validation<T> validation) {
        ...
        try {
            ...
            this.errors.addAll(e.getErrors().stream().map(ValidationError::from).toList());
        } catch (Throwable e) {
